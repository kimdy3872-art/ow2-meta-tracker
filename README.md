...
| 페이지 | 파일 | 설명 |
|---|---|---|
| 메인 | `main.py` | 티어, 포지션, 영웅 필터와 S/A/B/C/D 랭크 테이블, 장인 영웅 표시, 최신 패치노트/AI 분석 요약 |
...
| `score_strength` | 2축 사분면 라벨 (메타 지배/과열 주의/저평가 픽/비주류/보통) |
...
`score_strength`는 두 축의 조합으로 결정되는 사분면 라벨입니다.

| 존재감 | 성능 | 라벨 | 의미 |
|---|---|---|---|
| z ≥ 0.5 | z ≥ 0 | 메타 지배 | 많이 쓰이고 실제로 이김 |
| z ≥ 0.5 | z < 0 | 과열 주의 | 많이 쓰이거나 밴되지만 승률은 평균 이하 (함정 픽/불쾌함 밴 가능성) |
| z < 0.5 | z ≥ 0.5 | 저평가 픽 | 덜 쓰이지만 잘 이김 (장인 픽 후보) |
| z ≤ -0.5 | z < 0.5 | 비주류 | 메타에서 밀려남 |
| 그 외 | 그 외 | 보통 | — |
